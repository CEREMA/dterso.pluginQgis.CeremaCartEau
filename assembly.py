...
def assembleRasters(dlg, empriseVector, repRasterAssemblyList, ext_list, rasterAssembly):
    ...
    if os.path.exists(rasterAssembly):
        resp = QMessageBox.question(None, "Attention !!!", unicode(u"Le fichier raster %s existe déjà  voulez-vous l'écraser?"%(rasterAssembly)).encode('latin 1'), QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if resp == 16384 :
            try:
                os.remove(rasterAssembly)
            except:
                messErreur(dlg,u"Erreur le fichier raster %s ne peut pas être écrasé il est utilisé par un autre processus ou en lecture seul !!!"%(rasterAssembly))
                return -1
        else :
           return -1        
    ...
    if len(images_find_list) > 0 and os.path.isfile(images_find_list[0]) :
        ...
        messErreur(dlg,u"Erreur il n'y a pas de fichier image correspondant à l'emprise dans le(s) répertoire(s) : %s!!!"%(repRasterAssembly_str) )
        return -1
    ...
    if os.path.exists(merge_file_tmp) :
        ...
        messErreur(dlg,u"Erreur il n'y a pas de fichier assemblé %s à découper !!!"%(rasterAssembly))
        return -1
    ...
    for imagefile in glob.glob(repertory + os.sep + '*.*'):
        ...
        if imagefile.rsplit('.',1)[1] in extension_list :
            try:
                ...
                messErreur(dlg,u"Erreur Impossible d'ouvrir le fichier : %s !!!"%(imagefile))
...
def cutImageByVector(dlg, cut_shape_file, input_image, output_image, data_type, no_data_value, epsg):
    ...
    # Attention BUG QGIS report  #12502 see  => https://issues.qgis.org/issues/12502 et => http://osgeo-org.1560.x6.nabble.com/release-file-after-processing-runalg-td5318609.html
    # see https://gis.stackexchange.com/questions/231551/processing-gdalogrcliprasterbymasklayer-function-runs-with-no-results
    """
    # Découpage par gdal cliprasterbymasklayer
    try:
        processing.runalg('gdalogr:cliprasterbymasklayer', input_image, cut_shape_file, str(no_data_value), False, False, True, data_type, 0, 1, 1, 1, False, 0, False, "", output_image)
    except:
        messErreur(dlg,u"Erreur au cours du decoupage de l'image %s par le vecteur %s !!!"%(input_image, cut_shape_file))
        return
    """  
    ...
    try :
        ...
        messErreur(dlg,u"Erreur au cours du decoupage de l'image %s par le vecteur %s  avec gdalwarp!!!"%(input_image, cut_shape_file))
        return None
